...
public class ParserPoller extends StanfordCoreNLP{
	...
	private static PropertiesCredentials getAwsProperties() throws IOException {
		if (awsprops != null) {
			awsprops = new PropertiesCredentials(
	                ParserPoller.class.getResourceAsStream("AwsCredentials.properties")
			           );
		}
		return awsprops;
	}
     ...
     *
     */
    public ParserPoller(Properties props)  {
      this(props, (props == null || PropertiesUtils.getBool(props, "enforceRequirements", true)));
    }

    public ParserPoller(Properties props, boolean enforceRequirements)  {
     ...
     */
    public ParserPoller(String propsFileNamePrefix) {
      this(propsFileNamePrefix, true);
    }

    public ParserPoller(String propsFileNamePrefix, boolean enforceRequirements) {
	...
	/**
	  ...
	  public static void main(String[] args) throws IOException, ClassNotFoundException {
		  while (true) {
		    ...
		    Collection<File> bfc = new AWSBatchFileCollection(getClient());

		    if (bfc.size() == 0) {
		    	System.out.println("Error getting AWS files");
		    	System.exit(0);
		    }
		    ...
		    //
		    ParserPoller pipeline = new ParserPoller(props);
		    props = pipeline.getProperties();

		    pipeline.processFiles(bfc, numThreads);
		  }
	  }
    ...
    public void processFiles(final Collection<File> files, int numThreads) throws IOException {
        ...
        //for each file...
        int inqueue = 0;
        final TransferManager tm = getTransferManager();
        tm.getAmazonS3Client().setEndpoint("http://s3-us-west-2.amazonaws.com");
    ...
    protected void runNow(int numThreads, List<Runnable> toRun) {
        //--Run Jobs
        if(numThreads == 1){
